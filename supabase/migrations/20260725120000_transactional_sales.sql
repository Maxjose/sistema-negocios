...
begin
  ...
  select pm.name into v_payment_name
  ...
  from public.sales s where s.business_id = v_business_id;
  ...
  insert into public.sales (
    ...
    v_business_id, v_sale_number, v_subtotal, p_discount,
    v_subtotal - p_discount, v_total_cost,
    v_subtotal - p_discount - v_total_cost,
    p_payment_method_id, v_payment_name, nullif(trim(p_note), ''), v_user_id
  ...
  loop
    ...
    select c.name into v_category_name
      from public.categories c where c.id = v_product.category_id;
  ...
  insert into public.audit_logs (
    ...
    v_business_id, v_user_id, 'sale.created', 'sale', v_sale_id::text,
...
begin
  ...
  update public.sales set
    status = 'voided', voided_by = v_user_id, voided_at = now(),
    void_reason = trim(p_reason)
  where id = p_sale_id;
  insert into public.audit_logs (
    business_id, actor_user_id, action, entity_type, entity_id, after_data
  ) values (
    v_business_id, v_user_id, 'sale.voided', 'sale', p_sale_id::text,
