...
export default async function ReceivableDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const whatsappUrl = receivable.status === "open" && receivable.customers?.phone ? debtWhatsappUrl({ balance: Number(receivable.balance), customerName: receivable.customers.name, description: receivable.description, dueDate: receivable.due_date, phone: receivable.customers.phone }) : null;
