...
export default async function CustomersPage() {
  ...
  return <div className="mx-auto max-w-6xl">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold text-brand">Relaciones comerciales</p><h2 className="text-2xl font-bold">Clientes</h2><p className="mt-1 text-sm text-muted">Datos de contacto y estado de cada cliente.</p></div><div className="flex flex-wrap gap-2">{features.enable_credits && <Link className="inline-flex min-h-11 items-center gap-2 rounded-xl border bg-surface px-4 text-sm font-semibold text-foreground transition hover:bg-accent" href="/receivables"><CircleDollarSign className="size-4" /> Cuentas por cobrar</Link>}<CustomerCreateDialog /></div></div>
    <div className="mt-6">
