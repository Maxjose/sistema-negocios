...
export default async function ProductsPage({
  ...
  const [allProducts, features] = await Promise.all([
    getProducts(),
    getBusinessFeatures(),
  ]);
  ...
  return <div><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-muted">{features.use_stock ? "Catálogo e inventario actual" : "Catálogo de productos"}</p><h2 className="mt-1 text-2xl font-bold">Productos</h2></div><Link className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-brand px-4 text-sm font-semibold text-white" href="/products/new"><Plus className="size-4" /> Crear producto</Link></div>
    <form className={`mt-6 grid gap-3 rounded-2xl border bg-surface p-4 ${features.use_stock ? "sm:grid-cols-[1fr_12rem_auto]" : "sm:grid-cols-[1fr_auto]"}`}><input className="h-11 rounded-xl border px-3" defaultValue={filters.q} name="q" placeholder="Buscar por nombre o SKU" />{features.use_stock && <select className="h-11 rounded-xl border px-3" defaultValue={filters.stock ?? ""} name="stock"><option value="">Todo el inventario</option><option value="available">Disponibles</option><option value="low">Pocas unidades</option><option value="out">Agotados</option></select>}<button className="rounded-xl bg-brand px-4 text-sm font-semibold text-white" type="submit">Filtrar</button></form>
